const {inspect:inspectAccessibility}=require('./accessibility.cjs');
...
const {assignedFiles,assignmentFor}=require('./assignments.cjs');
const {readOutput,verifyOutput,requireVerified,createHandoff}=require('./delivery.cjs');
...
function portableAssignment(filename,original){return assignmentFor(session.folder,session.files.some(f=>f.path===original)?original:filename);}
...
async function accessibilityFor(filename,signature){
 selectedFile(filename);const before=await fs.stat(filename);
 if(signature!==before.size+':'+before.mtimeMs)throw Error('PDF changed. Reopen it before assessing accessibility.');
 const result=await inspectAccessibility(await fs.readFile(filename),path.basename(filename),{corrected:Object.values(session.outputs||{}).includes(filename)});
 const after=await fs.stat(filename);if(signature!==after.size+':'+after.mtimeMs)throw Error('PDF changed during accessibility assessment. Reopen it.');
 return result;
}
handle('accessibility',async payload=>accessibilityFor(payload.pdf,payload.signature));
...
handle('rescan',async changeFolder=>{
  ...
  session.records=session.records.map(r=>({...r,...assignedFiles(r,files,matchFiles)}));
  return publicSession();
});
...
handle('export-corrections',async payload=>{
  ...
  try{
    ...
    const result=await pdfJob({action:'correct',filename:payload.pdf,edits:payload.edits,rotations:payload.rotations,deskews:payload.deskews});
    ...
    const audit={application:'LayerProof',version:app.getVersion(),publisher:'Melken TechWork',exportedAt:new Date().toISOString(),source:payload.pdf,originalSource,output:destination,sourceSha256:result.sourceSha256,outputSha256:result.outputSha256,verification:result.verification,corrections:payload.edits,rotations:result.verification.rotations,deskews:result.verification.deskews};
    const row=selectedRecord(payload.key);
    // Reopening a completed document for correction invalidates its completion in Excel.
    // Do this before replacing the output so a failed workbook write cannot leave a new PDF marked complete.
    if(row.qcReviewed){
      const saved=await saveRegister(session.workbook,session.fingerprint,row.key,row.hasIssues,row.concerns,false,[],{reviewer:row.qcReviewer||settings().reviewer||'Reviewer',completedAt:'',pdfAssignment:portableAssignment(payload.pdf,originalSource)});
      session.fingerprint=saved.fingerprint;Object.assign(row,saved.records.find(r=>r.key===row.key));if(session.reviews[row.key])session.reviews[row.key].complete=false;
    }
...
handle('handoff',async()=>{
 if(pdfBusy||saving)throw Error('Wait for the current operation.');if(!session)throw Error('Open a project first.');saving=true;
 try{const picked=await dialog.showOpenDialog(window,{title:'Choose a parent folder for the client handoff package',properties:['openDirectory','createDirectory']});if(picked.canceled)return null;
 if(hash(await fs.readFile(session.workbook))!==session.fingerprint)throw Error('Workbook changed. Reopen the project before making a handoff.');
 return await createHandoff(picked.filePaths[0],session,app.getVersion());
...
handle('save-review',async payload=>{
  ...
  try{
    ...
    const originalSource=Object.keys(session.outputs).find(source=>session.outputs[source]===payload.pdf)||payload.pdf;
    if(checkedPages.length===payload.pageCount){
      if(payload.pendingChanges)throw Error('Export or undo pending PDF changes before completing this document.');
      const output=session.outputs[originalSource];if(output)await requireVerified(output);
    }
    const pdfAssignment=portableAssignment(payload.pdf,originalSource);
    const concerns=String(payload.concerns??'');
    const accessibility=await accessibilityFor(payload.pdf,payload.signature);
    ...
    const saved=await saveRegister(session.workbook,session.fingerprint,r.key,payload.hasIssues,concerns,checkedPages.length===payload.pageCount,legacyReviewedKeys,{reviewer,pdfAssignment,accessibility,completedAt:checkedPages.length===payload.pageCount?new Date().toISOString():''});
    const storedRows=new Map(saved.records.map(row=>[row.key,row]));
    for(const record of session.records)Object.assign(record,{accessibility:storedRows.get(record.key).accessibility,qcReviewed:storedRows.get(record.key).qcReviewed,qcReviewer:storedRows.get(record.key).qcReviewer,qcReviewedOn:storedRows.get(record.key).qcReviewedOn,pdfAssignment:storedRows.get(record.key).pdfAssignment});
    session.fingerprint=saved.fingerprint;r.hasIssues=payload.hasIssues;r.concerns=concerns;Object.assign(r,assignedFiles(r,session.files,matchFiles));
    session.reviews[r.key]={accessibility,id:r.id,pdf:payload.pdf,signature:payload.signature,checkedPages,pageCount:payload.pageCount,complete:checkedPages.length===payload.pageCount,hasIssues:r.hasIssues,concerns,savedAt:new Date().toISOString(),backup:saved.backup};
