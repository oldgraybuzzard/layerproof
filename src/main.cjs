...
app.setPath('userData', require('node:path').join(app.getPath('appData'),'PDF OCR QC'));
...
handle('rescan',async changeFolder=>{
  ...
  session.folder=folder;session.files=files;session.diagnostics=diagnostics;
...
handle('export-corrections',async payload=>{
  ...
  try{
    ...
    const selected=await dialog.showOpenDialog(window,{title:'Choose a separate folder for corrected PDFs',defaultPath:session.correctionOutputFolder||path.dirname(session.folder),properties:['openDirectory','createDirectory']});
    if(selected.canceled)return null;
    const outputFolder=await fs.realpath(selected.filePaths[0]);
    const destination=path.join(outputFolder,path.basename(payload.pdf));
    const source=await fs.realpath(payload.pdf);const normalize=p=>process.platform==='win32'?path.resolve(p).toLowerCase():path.resolve(p);
    if(normalize(path.dirname(source))===normalize(outputFolder))throw Error('Choose a separate output folder. The corrected PDF keeps its original filename.');
    session.correctionOutputFolder=outputFolder;
    ...
    const auditFile=destination+'.corrections.json';const created=[];
    try{
      const audit=await fs.open(auditFile,'wx');created.push(auditFile);try{await audit.writeFile(JSON.stringify({application:'LayerProof',version:app.getVersion(),publisher:'Melken TechWork',exportedAt:new Date().toISOString(),source:payload.pdf,output:destination,sourceSha256:result.sourceSha256,outputSha256:result.outputSha256,verification:result.verification,corrections:payload.edits,rotations:result.verification.rotations},null,2));}finally{await audit.close();}
      const output=await fs.open(destination,'wx');created.push(destination);try{await output.writeFile(result.bytes);}finally{await output.close();}
    }catch(e){for(const filename of created)await fs.unlink(filename).catch(()=>{});if(e.code==='EEXIST')throw Error('That output PDF or correction log already exists. Choose another output folder.');throw e;}
    return {path:destination,auditFile,verification:result.verification};
  }finally{pdfBusy=false;}
});
handle('read-pdf',async filename=>{selectedFile(filename);const stat=await fs.stat(filename);return {bytes:await fs.readFile(filename),signature:stat.size+':'+stat.mtimeMs};});
handle('save-review',async payload=>{
  ...
  try{
    ...
    const saved=await saveRegister(session.workbook,session.fingerprint,r.key,payload.hasIssues,concerns,checkedPages.length===payload.pageCount,legacyReviewedKeys);
    const storedRows=new Map(saved.records.map(row=>[row.key,row]));
    for(const record of session.records)record.qcReviewed=storedRows.get(record.key).qcReviewed;
