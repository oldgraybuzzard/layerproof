...
handle('save-review',async payload=>{
  ...
  try{
    ...
    const saved=await saveRegister(session.workbook,session.fingerprint,r.key,payload.hasIssues,concerns,checkedPages.length===payload.pageCount,legacyReviewedKeys,{reviewer,pdfAssignment,completedAt:checkedPages.length===payload.pageCount?new Date().toISOString():''});
    const storedRows=new Map(saved.records.map(row=>[row.key,row]));
    for(const record of session.records)Object.assign(record,{qcReviewed:storedRows.get(record.key).qcReviewed,qcReviewer:storedRows.get(record.key).qcReviewer,qcReviewedOn:storedRows.get(record.key).qcReviewedOn,pdfAssignment:storedRows.get(record.key).pdfAssignment});
