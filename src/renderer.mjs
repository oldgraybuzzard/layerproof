...
async function selectRecord(r){
  ...
  const previous=savedReview(r)?.pdf;const auto=previous&&(session.files.some(f=>f.path===previous)||Object.values(session.outputs||{}).includes(previous))?previous:r.exact.length===1?r.exact[0]:'';
...
async function exportCorrections(){
  ...
  try{const result=await window.qc.exportCorrections({pdf:pdfPath,signature,edits,rotations});if(!result){notice('Export cancelled. Staged corrections are still available.');return;}
    ...
    $('decision').value='Yes';dirtyChanged();exportedCorrections=new Map(edits.map(e=>[e.page+':'+e.index,e]));pendingCorrections.clear();exportedRotations=new Map(viewRotations);if(result.path===pdfPath){const savedChecks=[...checked],savedPage=pageNumber,savedFindings=new Map(pageFindings);await openPDF(result.path);checked=new Set(savedChecks);pageFindings=savedFindings;pageNumber=savedPage;await renderPage();}scheduleDraft();notice(`Corrected copy saved: ${result.path}. Change log saved alongside it. The original file is unchanged; save the review to record the export in Excel.`);
  }finally{busy=false;updateCoverage();}
}
...
function draftPayload(){if(!record||!pdf)return null;return {key:record.key,id:record.id,pdf:pdfPath,signature,baseIssues:record.hasIssues,baseConcerns:record.concerns,baseReviewed:record.qcReviewed,baseReviewer:record.qcReviewer,baseReviewedOn:record.qcReviewedOn,decision:$('decision').value,concerns:$('concerns').value,reviewer:$('reviewer').value,checked:[...checked],pageNumber,pageFindings:[...pageFindings],pendingCorrections:[...pendingCorrections],exportedCorrections:[...exportedCorrections],viewRotations:[...viewRotations],exportedRotations:[...exportedRotations],dirty};}
...
async function offerRecovery(){const draft=await window.qc.recover();if(!draft)return;heldDraft=true;const dialog=$('recovery');dialog.oncancel=event=>event.preventDefault();$('recovery-message').textContent=`Document ${draft.id}, saved ${new Date(draft.savedAt).toLocaleString()}. Restore concerns, page checks, and pending PDF changes?`;$('recovery-notes').value=draft.concerns||'';dialog.showModal();return new Promise(resolve=>{
 ...
 $('restore-draft').onclick=async()=>{try{await window.qc.validateDraft(draft);recovering=true;heldDraft=false;dirty=false;await selectRecord(session.records.find(r=>r.key===draft.key));if(pdfPath!==draft.pdf)await openPDF(draft.pdf);if(!pdf)throw Error('Cannot restore until the PDF is loaded.');$('decision').value=draft.decision;$('concerns').value=draft.concerns;$('reviewer').value=draft.reviewer||$('reviewer').value;checked=new Set(draft.checked.filter(p=>p>=1&&p<=pdf.numPages));pageFindings=new Map(draft.pageFindings);pendingCorrections=new Map(draft.pendingCorrections);exportedCorrections=new Map(draft.exportedCorrections);viewRotations=new Map(draft.viewRotations);exportedRotations=new Map(draft.exportedRotations);dirty=draft.dirty;$('review-state').textContent=dirty?'Restored unsaved review':'PDF changes restored';pageNumber=Math.min(pdf.numPages,Math.max(1,draft.pageNumber));await renderPage();dialog.close();notice('Draft restored. Save the review and export pending PDF changes when ready.');resolve();}catch(e){heldDraft=true;$('recovery-message').textContent=e.message;}finally{recovering=false;}};
});}
...
$('open-corrected').onclick=guarded(async()=>{if(!await mayLeave())return;const target=$('open-corrected').dataset.path;dirty=false;await openPDF(target);notice('Viewing corrected copy. Further exports update this copy with a backup.');});
