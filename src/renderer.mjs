...
import {nextCheckedPage,nextUncheckedPage,sortByPageCount,queueTarget,paneShare,matchingTextObject,mergedCorrections,pendingRotationCount} from './review-controls.mjs';
...
async function addConcernAndAdvance(button){if(!button||button.disabled)return;addConcern(button.dataset.issue);checked.add(pageNumber);dirtyChanged();updateCoverage();const next=nextCheckedPage(pageNumber,pdf.numPages);if(next!==null)await navigate(next);}
