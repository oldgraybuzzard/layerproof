...
import {matchingReview,isComplete,queueRows,nextPending} from './queue.mjs';
...
let viewRotations=new Map(),exportedRotations=new Map();
const pendingPDFChanges=()=>pendingCorrections.size+pendingRotationCount(viewRotations,exportedRotations);
...
function dirtyChanged(){dirty=true;$('review-state').textContent='Unsaved changes';}
...
async function selectRecord(r){
  ...
  const previous=savedReview(r)?.pdf;const auto=previous&&session.files.some(f=>f.path===previous)?previous:r.exact.length===1?r.exact[0]:'';
  $('pdf-file').value=auto;$('mapping-note').textContent=r.exact.length>1?'Multiple exact matches: choose the correct file.':r.suggested.length&&!auto?'Possible filename match: confirm the correct PDF for this row.':r.duplicate?'Saves affect this Excel row only.':'';
...
async function renderPage(){
  ...
  $('paper').style.width=viewport.width+'px';$('paper').style.height=viewport.height+'px';$('overlay').replaceChildren();$('ocr-text').replaceChildren();segments=[];pageText='';selected=-1;
...
async function save(next=false){
  ...
  try{const result=await window.qc.save({key:record.key,pdf:pdfPath,signature,pageCount:pdf.numPages,checkedPages:[...checked],hasIssues:$('decision').value,concerns:$('concerns').value});session=result.session;record=session.records.find(r=>r.key===record.key);dirty=false;$('review-state').textContent=checked.size===pdf.numPages?'Review complete':'Findings saved';$('last-saved').textContent='Saved '+new Date().toLocaleTimeString()+'. Backup: '+result.backup;queue();notice(result.warning||(next?(pendingPDFChanges()?'Document saved to Excel. Export your staged PDF corrections before moving to the next document.':'Document saved to Excel.'):`All pending review changes saved to Excel. ${pendingPDFChanges()?'Export your staged PDF corrections before closing.':'You can close the app.'}${checked.size===pdf.numPages?' Completed review hidden from the Needs review queue.':' This partial review remains in the queue.'}`),!!result.warning);
...
async function openProject(){if(busy||!await mayLeave())return;const result=await window.qc.open();if(!result)return;session=result;record=null;dirty=false;$('project').textContent=session.workbook.split(/[\\/]/).pop()+' / '+session.folder.split(/[\\/]/).pop();$('project').title=session.workbook+'\n'+session.folder;queue();$('filter').value='pending';const first=queueRows(session.records,session.reviews)[0]||session.records[0];if(first)await selectRecord(first);}
...
function updateCorrectionControls(){
  ...
  $('correction-count').textContent=pendingPDFChanges()?`${pendingCorrections.size} text correction(s), ${pendingRotationCount(viewRotations,exportedRotations)} page rotation(s) pending export. The original PDF is unchanged.`:'';
}
...
function stageCorrection(remove=false){
  ...
  $('correction-dialog').close();updateCorrectionControls();notice((remove?'Text deletion staged. ':'Correction staged. ')+'Export a corrected PDF to apply it; the original text remains unchanged.');
}
async function exportCorrections(){
  if(busy||!pendingPDFChanges())return;busy=true;updateCoverage();const edits=mergedCorrections(exportedCorrections,pendingCorrections);const rotations=[...viewRotations].filter(([,degrees])=>degrees).map(([page,degrees])=>({page,degrees}));if(!edits.length&&!rotations.length){pendingCorrections.clear();exportedCorrections.clear();exportedRotations=new Map(viewRotations);busy=false;updateCoverage();notice('All corrections were reverted. The original PDF already contains the requested text and orientation.');return;}notice('Creating a corrected copy and verifying its text and page appearance…');
  try{const result=await window.qc.exportCorrections({pdf:pdfPath,signature,edits,rotations});if(!result){notice('Export cancelled. Staged corrections are still available.');return;}
    const name=result.path.split(/[\\/]/).pop();for(const page of new Set([...edits.map(e=>e.page),...rotations.map(r=>r.page)])){$('concerns').value+=($('concerns').value.trim()?'\n':'')+`Page ${page}: PDF corrections exported to ${name}; original PDF retained.`;}
    $('decision').value='Yes';dirtyChanged();exportedCorrections=new Map(edits.map(e=>[e.page+':'+e.index,e]));pendingCorrections.clear();exportedRotations=new Map(viewRotations);notice(`Corrected copy saved: ${result.path}. Change log saved alongside it. The original file is unchanged; save the review to record the export in Excel.`);
  }finally{busy=false;updateCoverage();}
}
...
window.addEventListener('beforeunload',e=>{if(dirty||pendingPDFChanges()){e.preventDefault();e.returnValue='';}});
...
await guarded(async()=>{session=await window.qc.initial();if(session){$('project').textContent=session.workbook.split(/[\\/]/).pop()+' / '+session.folder.split(/[\\/]/).pop();queue();const first=queueRows(session.records,session.reviews)[0]||session.records[0];if(first)await selectRecord(first);}updateCoverage();})();
