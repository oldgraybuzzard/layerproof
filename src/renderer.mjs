...
import {nextCheckedPage,nextUncheckedPage,sortByPageCount,queueTarget,paneShare,matchingTextObject,mergedCorrections,pendingRotationCount,parsePageSelection} from './review-controls.mjs';
pdfjs.GlobalWorkerOptions.workerSrc=new URL('../node_modules/pdfjs-dist/build/pdf.worker.mjs',import.meta.url).href;
const $=id=>document.getElementById(id);
let session=null,record=null,pdf=null,pdfPath='',signature='',pageNumber=1,zoom=1,pageText='',segments=[],selected=-1,checked=new Set(),renderedPages=new Set(),dirty=false,loadVersion=0,renderVersion=0,renderTask=null,loadingTask=null,busy=false,refs={},pageFindings=new Map();
let viewRotations=new Map(),exportedRotations=new Map(),viewDeskews=new Map(),exportedDeskews=new Map(),outputVerified=null,correctedOpen=false,viewedSha256='';
let pageCounts=new Map(),pageCountVersion=0;
...
function knownPageCount(r){return pageCounts.get(r.key)??session.reviews[r.key]?.pageCount;}
async function loadQueuePageCounts(){
  const version=++pageCountVersion,currentSession=session,files=new Map();
  for(const r of session.records){const filename=session.reviews[r.key]?.pdf||(r.exact.length===1?r.exact[0]:null);if(filename){if(!files.has(filename))files.set(filename,[]);files.get(filename).push(r.key);}}
  for(const [filename,keys] of files){
    if(version!==pageCountVersion||session!==currentSession)return;
    try{const count=await window.qc.pageCount(filename);if(version!==pageCountVersion||session!==currentSession)return;for(const key of keys)pageCounts.set(key,count);queue();}catch{}
  }
}
function queue(){
  const search=$('search').value.toLowerCase(),filter=$('filter').value;
  let rows=queueRows(session.records,session.reviews,filter,search);
  rows=sortByPageCount(rows,new Map(rows.map(r=>[r.key,knownPageCount(r)])),$('queue-sort').value);
  const active=rows.findIndex(r=>r.key===record?.key);$('count').textContent=`${active<0?'—':active+1} / ${rows.length}`;$('queue').replaceChildren();
  for(const r of rows){const b=document.createElement('button');b.className='queue-item'+(record?.key===r.key?' active':'');b.dataset.key=r.key;
    ...
    const knownPages=knownPageCount(r);
    const subtitle=document.createElement('small');subtitle.textContent=`Excel row ${r.row}${knownPages?' · '+knownPages+' page'+(knownPages===1?'':'s'):''}${r.duplicate?' · duplicate ID':''}${!r.exact.length?' · '+(r.suggested.length?'assign PDF':'no match'):''}`;
...
function pageHasConcern(page){const prefix=`Page ${page}:`;return $('concerns').value.split('\n').some(line=>line.trim().startsWith(prefix));}
function updateCoverage(){$('handoff').disabled=busy||!session;updateCorrectionControls();for(const id of ['mark-page-checked','bulk-concern','rotate-left','rotate-right','deskew','change-output','open-corrected'])$(id).disabled=busy||!pdf;$('review-next-page').disabled=busy||!pdf||pageNumber>=pdf.numPages;$('next-unchecked').disabled=busy||!pdf||nextUncheckedPage(pageNumber,pdf.numPages,checked)===null;for(const id of ['decision','concerns','pdf-file','page-checked','page-clear'])$(id).disabled=busy;document.querySelectorAll('[data-issue]').forEach(b=>b.disabled=busy);const count=pdf?.numPages||0,needsReview=!!pdf&&!checked.has(pageNumber)&&pageHasConcern(pageNumber);$('coverage').textContent=`${checked.size} of ${count} pages checked${count&&checked.size===count?' · all pages checked':''}`;$('page-checked').checked=checked.has(pageNumber);$('page-review-status').textContent=!pdf?'':checked.has(pageNumber)?`Page ${pageNumber} checked`:needsReview?`Page ${pageNumber}: concern recorded; visual review required`:`Page ${pageNumber} not checked`;$('page-review-status').classList.toggle('needs-review',needsReview);$('save').disabled=$('save-next').disabled=!pdf||busy;$('prev').disabled=!pdf||pageNumber<=1;$('next').disabled=!pdf||pageNumber>=count;$('page-number').disabled=!pdf;}
function clearViewer(){showAccessibility(null);outputVerified=null;correctedOpen=false;viewDeskews=new Map();exportedDeskews=new Map();$('pdf-kind').textContent='No PDF open';$('open-corrected').hidden=true;viewRotations=new Map();exportedRotations=new Map();pendingCorrections=new Map();exportedCorrections=new Map();pdf=null;pageText='';segments=[];selected=-1;refs={};pageFindings=new Map();checked=new Set();renderedPages=new Set();$('canvas').width=0;$('canvas').height=0;$('paper').style.width='0px';$('paper').style.height='0px';$('overlay').replaceChildren();$('ocr-text').replaceChildren();$('empty').hidden=false;$('text-warning').hidden=true;$('selected-text').textContent='Nothing selected';$('flag-selection').disabled=true;$('page-count').textContent='/ —';$('text-count').textContent='No page loaded';$('reference').value='';$('metrics').textContent='';updateCoverage();}
...
async function openPDF(filename){
  ...
  const loaded=await loadingTask.promise;if(version!==loadVersion){await loaded.destroy();return;}pdf=loaded;pageCounts.set(record.key,pdf.numPages);queue();
...
async function renderPage(){
  ...
  renderedPages.add(pageNumber);$('text-count').textContent=segments.length+' text segments';$('page-number').value=pageNumber;$('zoom-label').textContent=Math.round(zoom*100)+'%';$('reference').value=refs[pageNumber]||'';$('metrics').textContent='';updateCoverage();
}
...
function markPageChecked(page=pageNumber){if(!renderedPages.has(page)){notice(`Open page ${page} in the viewer before marking it checked.`,true);return false;}checked.add(page);return true;}
function finishPageWithFindings(){if((pageFindings.get(pageNumber)||[]).some(entry=>$('concerns').value.includes(entry))&&markPageChecked()){dirtyChanged();updateCoverage();}}
function markPageClear(){if(!pdf)return;const prefix=`Page ${pageNumber}:`;if($('concerns').value.split('\n').some(line=>line.trim().startsWith(prefix))){notice('This page already has concerns. Remove or correct those entries before marking it clear.',true);return;}if(!markPageChecked())return;pageFindings.delete(pageNumber);dirtyChanged();updateCoverage();notice(`Page ${pageNumber} marked checked with no issues. The document decision is unchanged.`);}
...
async function goToNextUnchecked(){if(!pdf||busy)return;const target=nextUncheckedPage(pageNumber,pdf.numPages,checked);if(target===null){notice('Every other page is already checked.');return;}await navigate(target);}
async function addConcernAndAdvance(button){if(!button||button.disabled)return;addConcern(button.dataset.issue);if(!markPageChecked())return;dirtyChanged();updateCoverage();const next=nextCheckedPage(pageNumber,pdf.numPages);if(next!==null)await navigate(next);}
...
async function openProject(resume=false){if(busy||!await mayLeave())return;const result=await (resume===true?window.qc.resume():window.qc.open());if(!result)return;session=result;record=null;pageCounts=new Map();pageCountVersion++;dirty=false;$('project').textContent=session.workbook.split(/[\\/]/).pop()+' / '+session.folder.split(/[\\/]/).pop();$('project').title=session.workbook+'\n'+session.folder;$('filter').value='pending';queue();const first=queueRows(session.records,session.reviews)[0]||session.records[0];if(first)await selectRecord(first);void loadQueuePageCounts();await offerRecovery();}
...
async function refreshDiscovery(changeFolder){
  ...
  session=result;pageCounts=new Map();pageCountVersion++;dirty=false;$('project').textContent=session.workbook.split(/[\\/]/).pop()+' / '+session.folder.split(/[\\/]/).pop();$('project').title=session.workbook+'\n'+session.folder;queue();
  const next=session.records.find(r=>r.key===current)||session.records[0];if(next)await selectRecord(next);void loadQueuePageCounts();await showDiscovery();
}
...
$('review-next-page').onclick=guarded(async()=>{if(!pdf||busy)return;const next=nextCheckedPage(pageNumber,pdf.numPages);if(next===null||!markPageChecked())return;dirtyChanged();updateCoverage();await navigate(next);});
$('next-unchecked').onclick=guarded(goToNextUnchecked);
...
$('open').onclick=$('open-empty').onclick=guarded(openProject);$('search').oninput=$('filter').onchange=$('queue-sort').onchange=()=>session&&queue();
...
$('page-checked').onchange=()=>{if(!pdf){$('page-checked').checked=false;return;}if($('page-checked').checked){if(!markPageChecked()){updateCoverage();return;}}else checked.delete(pageNumber);dirtyChanged();updateCoverage();};
...
const bulkButton=document.createElement('button'),pageReviewStatus=document.createElement('p'),bulkDialog=document.createElement('dialog');bulkButton.id='bulk-concern';bulkButton.textContent='Apply concern to pages...';bulkButton.disabled=true;pageReviewStatus.id='page-review-status';pageReviewStatus.className='page-review-status';pageReviewStatus.setAttribute('role','status');$('review-next-page').after(bulkButton,pageReviewStatus);bulkDialog.id='bulk-concern-dialog';bulkDialog.setAttribute('aria-labelledby','bulk-concern-title');bulkDialog.innerHTML='<h2 id="bulk-concern-title">Apply concern to pages</h2><p>Add the same concern to several pages. Each page must still be opened and checked in the viewer.</p><label for="bulk-pages">Pages</label><input id="bulk-pages" placeholder="2-6, 9" inputmode="numeric"><label for="bulk-issue">Concern</label><select id="bulk-issue"></select><label for="bulk-comment">Additional detail</label><textarea id="bulk-comment" rows="3" maxlength="2000" placeholder="Optional detail, or enter the full custom comment"></textarea><p id="bulk-error" class="dialog-error" role="alert"></p><div><button id="apply-bulk-concern" class="primary">Apply concern</button><button id="cancel-bulk-concern">Cancel</button></div>';document.body.append(bulkDialog);
const bulkIssue=$('bulk-issue');bulkIssue.replaceChildren(new Option('Choose a concern...',''),...issueButtons.map(button=>new Option(button.textContent,button.dataset.issue)),new Option('Custom comment','__custom__'));
bulkButton.onclick=()=>{$('bulk-pages').value=String(pageNumber);bulkIssue.value='';$('bulk-comment').value='';$('bulk-error').textContent='';bulkDialog.showModal();$('bulk-pages').select();};$('cancel-bulk-concern').onclick=()=>bulkDialog.close();
$('apply-bulk-concern').onclick=()=>{try{const pages=parsePageSelection($('bulk-pages').value,pdf?.numPages||0),detail=$('bulk-comment').value.trim(),issue=bulkIssue.value,comment=issue==='__custom__'?detail:issue+(detail?': '+detail:'');if(!comment)throw Error('Choose a concern or enter a custom comment.');const existing=new Set($('concerns').value.split('\n').map(line=>line.trim()).filter(Boolean)),entries=[];for(const page of pages){const entry=`Page ${page}: ${comment}`;if(!existing.has(entry)){entries.push(entry);existing.add(entry);const findings=pageFindings.get(page)||[];findings.push(entry);pageFindings.set(page,findings);}}$('concerns').value+=($('concerns').value.trim()&&entries.length?'\n':'')+entries.join('\n');$('decision').value='Yes';dirtyChanged();updateCoverage();bulkDialog.close();notice(`${entries.length||'No new'} concern${entries.length===1?'':'s'} applied. ${pages.filter(page=>!checked.has(page)).length} page${pages.filter(page=>!checked.has(page)).length===1?'':'s'} still require visual review; use Next unchecked to verify them.`);}catch(error){$('bulk-error').textContent=error.message;}};
issueButtons.slice(0,9).forEach((button,index)=>{button.title=`Alt+${index+1}; add Shift to continue`;button.setAttribute('aria-keyshortcuts',`Alt+${index+1} Shift+Alt+${index+1}`);});
async function moveInQueue(move){if(!session||busy)return;const filtered=queueRows(session.records,session.reviews,$('filter').value,$('search').value),rows=sortByPageCount(filtered,new Map(filtered.map(r=>[r.key,knownPageCount(r)])),$('queue-sort').value);const target=queueTarget(rows,record?.key,move);if(target&&target.key!==record?.key)await selectRecord(target);}
function typingTarget(target){return target instanceof Element&&!!target.closest('input, textarea, select, [contenteditable="true"]');}
document.addEventListener('keydown',e=>{
  if(e.defaultPrevented||e.repeat||document.querySelector('dialog[open]'))return;
  const key=e.key.toLowerCase();
  if((e.metaKey||e.ctrlKey)&&key==='s'){e.preventDefault();guarded(()=>save(e.shiftKey))();return;}
  if(typingTarget(e.target)||e.metaKey||e.ctrlKey)return;
  if(e.altKey&&/^Digit[1-9]$/.test(e.code)){const button=issueButtons[Number(e.code.at(-1))-1];if(button&&!button.disabled){e.preventDefault();e.shiftKey?guarded(()=>addConcernAndAdvance(button))():button.click();}return;}
  if(e.altKey)return;
  if(e.key==='ArrowLeft'||e.key==='ArrowRight'){if(!pdf)return;e.preventDefault();guarded(()=>navigate(pageNumber+(e.key==='ArrowLeft'?-1:1)))();return;}
  if(key==='n'){e.preventDefault();$('review-next-page').click();return;}
  if(key==='u'){e.preventDefault();$('next-unchecked').click();return;}
  if(key==='c'){e.preventDefault();$('page-clear').click();return;}
  if(key==='g'){e.preventDefault();$('page-number').focus();$('page-number').select();return;}
...
await guarded(async()=>{const preferences=await window.qc.preferences();$('reviewer').value=preferences.reviewer||'';$('resume').hidden=!preferences.lastProject;$('resume').title=preferences.lastProject?.workbook||'';session=await window.qc.initial();if(session){$('project').textContent=session.workbook.split(/[\\/]/).pop()+' / '+session.folder.split(/[\\/]/).pop();queue();const first=queueRows(session.records,session.reviews)[0]||session.records[0];if(first)await selectRecord(first);void loadQueuePageCounts();await offerRecovery();}updateCoverage();})();

$('mark-page-checked').onclick=()=>{if(!pdf||busy||!markPageChecked())return;dirtyChanged();updateCoverage();notice('Page '+pageNumber+' marked checked. Save to record your review.');};
...
$('verify-output').onclick=guarded(async()=>{if(pendingPDFChanges())throw Error('Export pending changes first.');busy=true;updateCoverage();try{outputVerified=await window.qc.verifyOutput({pdf:pdfPath,signature,sha256:viewedSha256,checkedPages:[...checked],reviewer:$('reviewer').value});dirtyChanged();notice('Corrected copy verified. Save to workbook to record completion.');}finally{busy=false;updateCoverage();}});
