...
Create an environment named `windows-signing` in the private LayerProof repository, restrict it to `main`, and configure its secrets and variables below. Set `SIGNING_PUBLISHER` to the exact certificate publisher name. Use the verified identity for Melken TechWork, not an invented subject name.
