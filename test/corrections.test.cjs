...
test('bulk page selection parses ranges, sorts, deduplicates and rejects invalid pages',async()=>{const {parsePageSelection}=await import('../src/review-controls.mjs');assert.deepEqual(parsePageSelection('5, 2-4, 3',8),[2,3,4,5]);assert.deepEqual(parsePageSelection('1, 8',8),[1,8]);assert.throws(()=>parsePageSelection('',8),/at least one/);assert.throws(()=>parsePageSelection('4-2',8),/lower page/);assert.throws(()=>parsePageSelection('2, 9',8),/between 1 and 8/);assert.throws(()=>parsePageSelection('all',8),/such as/);});
